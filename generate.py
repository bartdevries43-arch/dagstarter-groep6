...
# -*- coding: utf-8 -*-
"""
Dagstarter / automatiseren-boekje voor groep 6.
...
def gen_automatiseren(rnd, t):
    ...
    items = []
    # ---- keersommen ---- (groep 6: tafels 1-10 automatiseren, later beetje verder)
    for _ in range(8):
        if t < 0.30:
            a, b = rnd.randint(1, 10), rnd.randint(1, 5)
        elif t < 0.60:
            a, b = rnd.randint(1, 10), rnd.randint(2, 10)
        elif t < 0.80:
            a, b = rnd.randint(2, 10), rnd.randint(2, 10)
        else:
            if rnd.random() < 0.5:
                a, b = rnd.randint(11, 15), rnd.randint(2, 9)
            else:
                a, b = rnd.choice([10, 20, 100]), rnd.randint(2, 9)
        items.append({"vraag": f"{a} × {b} =", "antwoord": str(a * b)})

    # ---- deelsommen (uit de tafels, rest pas later) ----
    for _ in range(6):
        if t < 0.30:
            deler, quot = rnd.randint(2, 5), rnd.randint(2, 10)
            rest = 0
        elif t < 0.60:
            ...
            rest = rnd.choice([0, 0, 0, rnd.randint(1, deler - 1)])
        else:
...
def _wissel_breuken(rnd, t):
    """Deel van een geheel met makkelijke breuken (groep 6): 1/2, 1/4, 3/4, 1/10."""
    breuken = [(1, 2), (1, 4), (3, 4), (1, 10), (1, 5)]
    items = []
    for _ in range(4):
        teller, noemer = rnd.choice(breuken)
        geheel = rnd.randint(2, 10) * noemer  # deelbaar, mooie uitkomst
...
def gen_wissel(rnd, t, dagnummer):
    # groep 6: geen procenten (dat is groep 6); breuken pas later erbij
...
LEESWOORDEN = [
    ...
    "tijger", "hond", "poes", "vogel", "bloem", "water", "tafel", "stoel",
    "raam", "deur", "bal", "fiets", "zomer", "winter", "vandaag", "morgen",
    # iets langer (2-3 lettergrepen, klankgroepen)
    "voetbal", "muziek", "gitaar", "meester", "juffrouw", "banaan", "tomaat",
    "olifant", "konijn", "trein", "ijsje", "eiland", "gordijn", "kabouter",
    "familie", "kalender", "vakantie", "paraplu", "chocola", "machine",
    "spannend", "grappig", "gezellig", "vrolijk", "moeilijk", "eerlijk",
    "prachtig", "opnieuw", "misschien", "eigenlijk", "natuurlijk", "verjaardag",
    # moeilijker (langere en samengestelde woorden)
    "computer", "station", "politie", "museum", "kilometer", "boterham",
    "ziekenhuis", "huiswerk", "voetbalclub", "schoolplein", "brandweerman",
    "verkeerslicht", "tandenborstel", "verrekijker", "boodschappen",
    "voorstelling", "gebeurtenis", "temperatuur", "informatie", "nieuwsgierig",
    "belangrijk", "gemakkelijk", "verschillende", "geschiedenis",
    "aardrijkskunde", "kampioenschap", "herfstvakantie", "sinterklaasfeest",
]
